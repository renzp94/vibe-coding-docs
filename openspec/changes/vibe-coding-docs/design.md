...
- ~~是否需要集成评论系统（如 Giscus）？~~ ✅ 已确定：使用 Giscus
- ~~是否需要多语言支持？~~ ✅ 已确定：不需要
- ~~是否需要版本控制（如不同版本的教程）？~~ ✅ 已确定：不需要

## 附加功能设计

### 内容变更追踪（Git Diff 视图）

**需求**: 内容更新时，需要标记新增/删除的内容，并支持悬浮查看更改时间

**实现方案**:
- 使用 Git 历史追踪每行内容的最后修改时间
- 在构建时通过 Git 命令获取每行的 `git blame` 信息
- 在 Markdown 渲染时注入数据属性 `data-last-modified`
- 通过 CSS 高亮最近修改的内容（新增：绿色背景，删除：红色删除线）
- 悬浮时显示 Tooltip 展示：作者、提交时间、提交信息

**技术实现**:
- 构建时运行 `git blame` 获取行级元数据
- VitePress 插件处理 Markdown，注入 blame 信息
- 前端组件渲染 Diff 标记和悬浮提示
- 可选：展示当前页面与上一版本的完整 Diff 视图
