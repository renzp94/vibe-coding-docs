...
## 8. 内容变更追踪 (content-change-tracking)

- [ ] 8.1 创建 Git blame 数据获取工具
  - 编写脚本获取每行内容的 git blame 信息
  - 解析 commit hash、作者、时间戳、提交信息
  - 将数据保存为 JSON 格式供构建使用
- [ ] 8.2 开发 VitePress 插件
  - 创建 markdown-it 插件处理 blame 数据注入
  - 为每行内容添加 data-last-modified 属性
  - 支持配置高亮时间阈值（如 7 天内为「最近」）
- [ ] 8.3 实现变更标记样式
  - 新增内容：左侧绿色竖线 + 淡绿背景
  - 修改内容：左侧黄色竖线 + 淡黄背景
  - 删除内容（Diff 视图）：红色删除线 + 淡红背景
- [ ] 8.4 实现悬浮提示组件
  - 创建 Tooltip 组件展示提交信息
  - 显示：作者头像、用户名、提交时间、提交信息
  - 添加平滑动画和自适应定位
- [ ] 8.5 实现 Diff 视图模式
  - 创建 DiffViewer 组件
  - 支持对比两个 Git 版本
...
- [ ] 8.6 实现最近更新横幅
  ...
  - 支持关闭和本地存储记忆
- [ ] 8.7 集成到构建流程
  - 在 npm run docs:build 前执行 blame 数据收集
  - 确保 CI/CD 环境有 Git 历史可用
  - 处理浅克隆（shallow clone）的兼容性问题
